...
import CloudServiceBadgesCarousel from '../components/CloudServiceBadgesCarousel';
...
export default function CloudServicesPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const payload = { ...formData, source: 'cloud-services', path: typeof window !== 'undefined' ? window.location.pathname : '' };
  ...
  const reviews = [
    { content: 'Cloud migration done with zero downtime.', name: 'Client Review', rating: 5 },
    { content: 'Observability and security are top-notch.', name: 'Client Review', rating: 5 },
    { content: 'CI/CD improved our release speed.', name: 'Client Review', rating: 5 },
    { content: 'Cost optimization led to real savings.', name: 'Client Review', rating: 5 },
  ];
  ...
  useEffect(() => {
    const url = new URL('../icons/cloud page.json', import.meta.url).href;
    fetch(url)
      .then((res) => res.json())
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${darkMode ? 'dark' : ''}`}>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div className="md:col-span-2">
              ...
              {/* Removed About, practices chips, and CTA sections */}
            </div>
          ...
          <div className="space-y-12">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                <h2 className="text-5xl md:text-6xl font-extrabold text-[#D9E8FF] leading-tight mb-6">
                  Best <span className="text-[#8C75FF]">Cloud Services</span> Company in Hyderabad
                </h2>
                <p className="text-[#8892A6] mb-8">
                  We architect secure, scalable cloud solutions that reduce costs and boost reliability. From migration to DevOps, observability, and security hardening—we help you operate with confidence.
                </p>
            ...
            <motion.div className="mt-6 mx-auto max-w-5xl p-5 md:p-6 rounded-2xl border border-[#8C75FF] bg-[#131A2C]" initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.2 }} variants={listVariants}>
              <div className="grid sm:grid-cols-3 gap-3 md:gap-5">
                {[
                  { emoji: '☁️', title: 'Cloud Hosting' },
                  { emoji: '🔄', title: 'Auto Scaling' },
                  { emoji: '🛡️', title: 'Enhanced Security' },
                  { emoji: '💾', title: 'Data Backup & Recovery' },
                  { emoji: '⚙️', title: 'DevOps & Automation' },
                  { emoji: '🧩', title: 'Microservices Architecture' },
                  { emoji: '📡', title: 'Global CDN Support' },
                  { emoji: '🚀', title: 'High-Speed Deployment' },
                  { emoji: '📊', title: 'Performance Monitoring' },
                  { emoji: '🔗', title: 'API Gateway Integration' },
                  { emoji: '🧠', title: 'AI/ML Cloud Compute' },
                  { emoji: '🏗', title: 'Serverless Architecture' },
                  { emoji: '🛠', title: 'Infrastructure as Code (IaC)' },
                  { emoji: '🧭', title: 'Cloud Migration Support' },
                  { emoji: '🔍', title: 'Log Management' },
                  { emoji: '📈', title: 'Load Balancing' },
                  { emoji: '🔐', title: 'Zero-Trust Access Control' },
                  { emoji: '🔋', title: 'Resource Optimization' },
                ].map((f) => (
      ...
      {/* Services we offer – Cloud (carousel, same UI as App Dev) */}
      <CloudServiceBadgesCarousel />
