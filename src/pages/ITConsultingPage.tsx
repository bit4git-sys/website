...
import ITConsultingBadgesCarousel from '../components/ITConsultingBadgesCarousel';
...
export default function ITConsultingPage() {
  ...
  const reviews = [
    { content: 'Strategic advice that drove measurable results.', name: 'Client Review', rating: 5 },
    { content: 'Clear roadmaps and execution support.', name: 'Client Review', rating: 5 },
    { content: 'Stakeholder communication was excellent.', name: 'Client Review', rating: 5 },
    { content: 'Transformation plan delivered on schedule.', name: 'Client Review', rating: 5 },
  ];
  ...
  useEffect(() => {
    const url = new URL('../icons/IT consulting page.json', import.meta.url).href;
    fetch(url)
      .then((res) => res.json())
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${darkMode ? 'dark' : ''}`}>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div className="md:col-span-2">
              ...
              {/* Removed About, engagements chips, and CTA sections */}
            </div>
          ...
          <div className="space-y-12">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                <h2 className="text-5xl md:text-6xl font-extrabold text-[#D9E8FF] leading-tight mb-6">
                  Best <span className="text-[#8C75FF]">IT Consulting</span> Company in Hyderabad
                </h2>
                <p className="text-[#8892A6] mb-8">
                  We help you plan and execute technology strategies—roadmaps, architecture, and transformation initiatives—that deliver measurable business outcomes.
                </p>
            ...
            <motion.div className="mt-6 mx-auto max-w-5xl p-5 md:p-6 rounded-2xl border border-[#8C75FF] bg-[#131A2C]" initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.2 }} variants={listVariants}>
              <div className="grid sm:grid-cols-3 gap-3 md:gap-5">
                {[
                  { emoji: '🧠', title: 'Technology Strategy Planning' },
                  { emoji: '📊', title: 'Business IT Alignment' },
                  { emoji: '⚙️', title: 'Enterprise Architecture Consulting' },
                  { emoji: '🔍', title: 'IT Assessment & Analysis' },
                  { emoji: '🪪', title: 'Risk & Compliance Advisory' },
                  { emoji: '🧩', title: 'Digital Transformation Consulting' },
                  { emoji: '🛡', title: 'Cybersecurity Consulting' },
                  { emoji: '☁️', title: 'Cloud Advisory & Planning' },
                  { emoji: '📈', title: 'Cost Optimization Strategy' },
                  { emoji: '🚀', title: 'IT Innovation & Scaling Roadmap' },
                  { emoji: '🔗', title: 'Vendor & Tools Evaluation' },
                  { emoji: '🧪', title: 'Proof of Concept (PoC) Building' },
                  { emoji: '📑', title: 'Policy & Framework Development' },
                  { emoji: '🔄', title: 'Process Improvement & Automation' },
                  { emoji: '📝', title: 'Technology Roadmap Blueprinting' },
                  { emoji: '🧬', title: 'Legacy System Modernization' },
                  { emoji: '📡', title: 'Infrastructure Design Consultation' },
                  { emoji: '🤝', title: 'Enterprise Collaboration Guidance' },
                  ].map((f) => (
      ...
      {/* Services we offer – IT Consulting (carousel matching App Dev UI) */}
      <ITConsultingBadgesCarousel />

      <section className="bg-[#0A0F1F]">
        <div className="container mx-auto px-6 py-16">
          <div className="grid md:grid-cols-2 gap-12 items-start">
            <div className="p-6 rounded-2xl bg-[#131A2C] border border-[#2B3561]">
