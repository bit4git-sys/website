...
import CybersecurityBadgesCarousel from '../components/CybersecurityBadgesCarousel';
...
export default function CybersecurityPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const payload = { ...formData, source: 'cybersecurity', path: typeof window !== 'undefined' ? window.location.pathname : '' };
    try {
  ...
  const reviews = [
    { content: 'Our security posture improved significantly.', name: 'Client Review', rating: 5 },
    { content: 'Rapid incident response and clear playbooks.', name: 'Client Review', rating: 5 },
    { content: 'Compliance and governance simplified.', name: 'Client Review', rating: 5 },
    { content: 'Proactive monitoring reduced risk.', name: 'Client Review', rating: 5 },
  ];
  ...
  useEffect(() => {
    const url = new URL('../icons/cybersecurity page.json', import.meta.url).href;
    fetch(url)
      .then((res) => res.json())
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${darkMode ? 'dark' : ''}`}>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div className="md:col-span-2">
              ...
              {/* Removed About, capabilities chips, and CTA sections */}
            </div>
          ...
          <div className="space-y-12">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                <h2 className="text-5xl md:text-6xl font-extrabold text-[#D9E8FF] leading-tight mb-6">
                  Best <span className="text-[#8C75FF]">Cybersecurity</span> Company in Hyderabad
                </h2>
                <p className="text-[#8892A6] mb-8">
                  We protect your business with modern security practices—threat detection, incident response, compliance, and ongoing monitoring—so you can operate securely and confidently.
                </p>
            ...
            <motion.div className="mt-6 mx-auto max-w-5xl p-5 md:p-6 rounded-2xl border border-[#8C75FF] bg-[#131A2C]" initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.2 }} variants={listVariants}>
              <div className="grid sm:grid-cols-3 gap-3 md:gap-5">
                {[
                  { emoji: '🛡', title: 'Threat Protection' },
                  { emoji: '🔍', title: 'Vulnerability Assessment' },
                  { emoji: '🧪', title: 'Penetration Testing' },
                  { emoji: '🕵️‍♂️', title: 'Digital Forensics & Investigation' },
                  { emoji: '🚨', title: 'Incident Detection & Response' },
                  { emoji: '🔐', title: 'Data Encryption & Privacy Control' },
                  { emoji: '🧰', title: 'SOC Monitoring & Management' },
                  { emoji: '🧬', title: 'Malware Analysis & Reverse Engineering' },
                  { emoji: '🔗', title: 'Network Security Architecture' },
                  { emoji: '📜', title: 'Security Policy & Compliance' },
                  { emoji: '🧱', title: 'Firewall & Endpoint Security' },
                  { emoji: '🕸', title: 'Web App Security Testing' },
                  { emoji: '🪪', title: 'Identity & Access Management (IAM)' },
                  { emoji: '📡', title: 'SIEM Deployment & Optimization' },
                  { emoji: '🧯', title: 'Ransomware Prevention & Recovery' },
                  { emoji: '🌐', title: 'Secure Cloud Infrastructure' },
                  { emoji: '⚠️', title: 'Risk Assessment & Governance' },
                  { emoji: '🔄', title: 'Red Team / Blue Team Exercises' },
                  { emoji: '🧩', title: 'Zero-Trust Implementation' },
                  { emoji: '📊', title: 'Security Analytics & Reporting' },
                ].map((f) => (
      ...
      {/* Services we offer – Cybersecurity (carousel matching Cloud/IT UI) */}
      <CybersecurityBadgesCarousel />
